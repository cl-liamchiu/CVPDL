...
def load_ckpt(ckpt_path):
    ...
    model_parts = torch.load(ckpt_path)
...
def run(meta, config, starting_noise=None):
    ...
    image_names = [meta["image_name"]]
    image_labels = [meta["image_label"]]
    for image_name, image_label, sample in zip(image_names, image_labels, samples_fake):
        img_name = image_name
        sample = torch.clamp(sample, min=-1, max=1) * 0.5 + 0.5
        sample = sample.cpu().numpy().transpose(1,2,0) * 255 
        sample = Image.fromarray(sample.astype(np.uint8))
        file_name = f"{image_label}_{img_name}"
        sample.save(os.path.join(output_folder, file_name))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--use_image", action='store_true', help="use DDIM instead. WARNING: I did not test the code yet")
    ...
    with open("../images_with_caption.json", "r") as f:
        images_with_caption = json.load(f)
    
    ckpt = args.ckpt_file ## 要改
    alpha_type = [0.3, 0.0, 0.7]
    save_folder_name=args.save_folder ## 要改
    meta_list = []

    # with open('finished_list.txt', 'r') as f:
    #     finished_list = f.read().splitlines()

    # finished_list = finished_list[280:]

    for image in images_with_caption:
        # if image['image'] in finished_list:
        #     continue

        # - - - - - - - - GLIGEN on image grounding for generation - - - - - - - - # 
        if args.use_image:
            image_path = f"../selected_images/{image['image']}"
            meta_list.append( dict(
                ckpt = ckpt,
                prompt = image['prompt1'], ## 要改
                images = [image_path],
                phrases = ["a " + image['label']] * len(image['bboxes']),
                locations = image['bboxes'],
    ...
    starting_noise = None
    for meta in tqdm(meta_list, desc="generating images"):
        run(meta, args, starting_noise)
        with open("finished_list.txt", "a") as f:
            f.write(meta['image_name'] + "\n")
